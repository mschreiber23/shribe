...
router.get('/previous', (req, res) => {
  ...
  let query = `
    ...
    WHERE ws.plan_id = ? AND ws.completed_at IS NOT NULL AND ws.user_id = ?
  `;
...
router.post('/', (req, res) => {
  ...
  // Allow global plans or plans owned by the user
  const plan = db.prepare('SELECT * FROM workout_plans WHERE id = ? AND (user_id = ? OR is_global = 1)').get(plan_id, req.userId);
  if (!plan) return res.status(404).json({ error: 'Plan not found' });

  const result = db.prepare(
    'INSERT INTO workout_sessions (user_id, plan_id, date, schedule_entry_id, notes) VALUES (?, ?, ?, ?, ?)'
  ).run(req.userId, plan_id, date, schedule_entry_id || null, notes || null);
...
router.put('/:id', (req, res) => {
  ...
  const session = db.prepare('SELECT * FROM workout_sessions WHERE id = ? AND user_id = ?').get(req.params.id, req.userId);
  if (!session) return res.status(404).json({ error: 'Session not found' });
...
router.put('/:sessionId/sets/:setId', (req, res) => {
  ...
  const setLog = db.prepare(`
    ...
    WHERE sl.id = ? AND sl.session_id = ? AND ws.user_id = ?
